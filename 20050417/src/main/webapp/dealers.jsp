...
<h1 style="padding: 50px;">딜러별 총 매출</h1>
...
ResultSet rs = stmt.executeQuery("SELECT DEALER_NAME, COUNT(*), SUM(s.SELL_PRICE) FROM TBL_SALES_202504 s, TBL_DEALER_202504 d WHERE s.DEALER_ID = d.DEALER_ID GROUP BY d.DEALER_NAME ORDER BY d.DEALER_NAME");
while(rs.next()) {
%>
<tr>
	<td><%=rs.getString(1)%></td>
	<td><%=rs.getString(2)%>건</td>
	<td><%=NumberFormat.getInstance().format(rs.getInt(3))%>만원</td>
</tr>
<% } %>
</table>
</form>
