...
SELECT m.M_NO, m.V_NAME, p.P_NAME, CASE WHEN m.V_TIME = '1' THEN '고졸' WHEN m.V_TIME = '2' THEN '학사' WHEN m.V_TIME = '3' THEN '석사' WHEN m.V_TIME = '4' THEN '박사' END, m.V_AREA, m.V_CONFIRM, substr(p.P_TEL1, 0, 2) || '-'|| p.P_TEL2 || '-' || p.P_TEL3 FROM TBL_MEMBER_202005 m, TBL_PARTY_202005 p WHERE m.P_CODE = p.P_CODE;
...
SELECT m.M_NO, m.V_NAME, p.P_NAME, COUNT(*) FROM TBL_MEMBER_202005 m, TBL_PARTY_202005 p, TBL_VOTE_202005 v WHERE m.M_NO = v.M_NO AND m.P_CODE = p.P_CODE GROUP BY m.M_NO, p.P_NAME, m.V_NAME ORDER BY COUNT(*) DESC;