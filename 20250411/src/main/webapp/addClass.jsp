...
<header>
	...
	<form action="classCtrl.jsp">
		<table>
			<tr>
				<td class="sub">수강월</td>
				<td class="sub"><input name="class_ym"><p></p>2022년03월 예)202203</td>
			</tr>
			<tr>
				<td class="sub">회원명</td>
				<td class="sub">
			...
			<tr>
				...
				<td class="sub">
					<select name="class_loc" id="mem_nm">
			...
			<tr>
				...
				<td class="sub">
					...
					<%while(rs3.next()){%>
						<option id="<%=rs3.getString("class_price")%>h" data-code="<%=rs3.getString("teacher_cd")%>" value="<%=rs3.getString("class_price")%>"><%=rs3.getString("class_nm")%></option>
					<%}%>
					</select>
					<input id="teacher_cd" name="teacher_cd" hidden/>
				</td>
			</tr>
			<tr>
				<td class="sub">수강료</td>
	...
	<script>
		...
		document.querySelector("#class_nm").addEventListener("change", (e) => {
			document.querySelector("#teacher_cd").value = document.getElementById(e.target.value + "h").dataset.code;
